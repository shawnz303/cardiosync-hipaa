...
In order to preserve the integrity of data that Iono stores, processes, or transmits for Customers, Iono implements strong intrusion detection tools and policies to proactively track and retroactively investigate unauthorized access. Iono currently utilizes [OSSEC](http://www.ossec.net/) to track file system integrity, monitor log data, and detect rootkit access.
...
* Iono firewalls monitor all incoming traffic to detect potential denial of service attacks. Suspected attack sources are blocked automatically. Additionally, our hosting provider actively monitors its network to detect denial of services attacks.
...
* Iono utilizes redundant firewall on network perimeters.
* Static IP addresses are used for Iono servers.
