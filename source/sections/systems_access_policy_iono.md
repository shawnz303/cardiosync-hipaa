...
* Requests for access to Iono Platform systems and applications is made formally to the VP of Engineering, Privacy Officer, or Security Officer.
* Access is not granted until receipt, review, and approval by the Iono Security Officer;
* The request for access is retained for future reference.
* All access to Iono systems and services are reviewed and updated on an bi-annual basis to assure proper authorizations are in place commensurate with job functions. Email security@iono.co
* Any Iono workforce member can request change of access by emailing security@iono.co.
...
* In the case of non-personal information, such as generic educational content, identification and authentication may not be required. This is the responsibility of Iono Customers to define, and not Iono.
...
* Generic accounts are not allowed on Iono systems.
...
* Iono maintains a minimum necessary approach to access to Customer data. As such, Iono, including all workforce members, does not readily have access to any ePHI.

## Access Authorization

* Role based access categories for each Iono system and application are pre-approved by the Security Officer or VP of Engineering.
* Iono utilizes hardware and software firewalls to segment data, prevent unauthorized access, and monitor traffic for denial of service attacks.
...
* Access to the Iono Platform systems and applications is controlled by requiring unique User Login ID’s and passwords for each individual user and developer.
...
* Shared accounts are not allowed within Iono systems or networks.
...
All workstations at Iono are company owned, and all are laptop Apple products running Mac operating system.
...
* All workstations are to have the following messages added to the lock screen and login screen: *This computer is owned by Iono, Inc. By logging in, unlocking, and/or using this computer you acknowledge you have seen, and follow, these policies (https://catalyze.io/policy/) and have completed this training (https://training.catalyze.io/). Please contact us if you have problems with this - privacy@catalyze.io.*

## Wireless Access Use

* Iono production systems are not accessible directly over wireless channels.
...
* Wireless networks managed within Iono non-production facilities (offices, etc) are secured with the following configurations:
	* All data in transit over wireless is encrypted using WPA2 encryption;
	* SSIDs are not broadcast;
	* Passwords are rotated on a regular basis, presently quarterly. This process is managed by the Iono Security Officer.
...
Iono does not use paper records for any sensitive information. Use of paper for recording and storing sensitive data is against Iono policies.

## Password Management

* User IDs and passwords are used to control access to Iono systems and may not be disclosed to anyone for any reason.
...
* On all production systems and application in the Iono environment, password configurations are set to require that passwords are a minimum of 8 character length, 90 day password expiration, account lockout after 5 invalid attempts, password history of last 4 passwords remembered, and account lockout after 15 minutes of inactivity.
...
Iono grants PaaS customer secure system access via VPN connections. This access is only to Customer-specific systems, no other systems in the environment. These connections are setup at customer deployment. These connections are secured and encrypted and the only method for customers to connect to Iono hosted systems.

In the case of data migration, Iono does, on a case by case basis, support customers in importing data. In these cases Iono support SCP assuring all data is secured and encrypted in transit.

In the case of an investigation, Iono will assist customers, at Iono's discretion, and law enforcement in forensics.
